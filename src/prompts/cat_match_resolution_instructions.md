...
[`cat_matching_instructions.md`](cat_matching_instructions.md) to `cat_mapping.json`. Resolve only
the records addressed by those instructions, preserve every unaffected mapping, and validate the
complete result before replacing `cat_mapping.json`.
...
Read `extraction.json`, `needs_invoice.json`, `cat_mapping.json`, and `cat_match_review.json` from
...
```json
{
  "26SEP02-NLF-2": {
    ...
    "resolution": "Match this extraction cat to Airtable cat recExample456. The paperwork name is its intake name."
  }
}
```
...
Every non-empty review entry must contain exactly `extraction_cat_display_name`, `airtable_id`,
`airtable_display_name`, `match_reason`, and `resolution`, and `resolution` must be a string.
...
1. Copy the complete existing `cat_mapping.json` result into memory. Do not rebuild unaffected
...
3. For an operator-directed match, add or update the extraction `cat_id` entry with the exact
   `airtable_cat_id` and `cat_name` from `needs_invoice.json`. Never copy a display name from the
   resolution text. Preserve `extraction_cat_display_name` exactly from the extraction record's
   `display_name`.
...
5. A confirmation of an already populated match may improve `match_reason`, but must not otherwise
   ...
   `cat_mapping.json`; its operator-directed outcome remains recorded in `cat_match_review.json`.
6. An instruction that an Airtable cat has no extraction counterpart does not create a
   `cat_mapping.json` entry: that file remains keyed only by extraction `cat_id`. Preserve the
...
Before changing `cat_mapping.json`, verify all of the following:

- every top-level key is an extraction `cat_id`, and only matched extraction cats appear;
- every entry contains exactly `extraction_cat_display_name`, `airtable_id`,
...
- every `airtable_id` exists in `needs_invoice.json` and appears at most once;
...
- every unaffected mapping is byte-for-byte equivalent at the entry level; and
- any requested Airtable split completed and passed its post-split checks before the mapping file
  is written.

When all checks pass, atomically replace `cat_mapping.json` in its existing run directory. This
...
record. If the resolved mapping is already identical, make no write and report that no update was
needed.
